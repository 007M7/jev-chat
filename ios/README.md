...
**每个阶段都有硬性时间预算**，超了就放弃这一轮并说明是哪一步超时，不会一直等下去：

| 阶段 | 预算 | 取值依据 |
|---|---|---|
| 感知 | 40s | 实测最慢 15.5s，留 2.5 倍余量 |
| 判断 | 25s | 实测最慢 1.7s |
| 起草 | 35s | 实测最慢 7.0s |
| 排序 | 20s | 实测最慢 1.1s |

预算写在 `tools/jev/providers.json` 的 `role_timeouts` 里（可用覆盖层改）。另外还有一道
150 秒的硬上限做安全网：一旦超过就强制结束这一轮，因为分析期间新帧是被丢掉的——
**一次卡住会把后面所有消息一起堵住**，这比超时本身更糟。
...
- **通知可能被系统静默拦掉**。系统层的通知授权一旦被拒（免费签名重装后 Bundle ID 若被换掉，
  系统会当成全新 App，授权要重新给），`UNUserNotificationCenter.add` 是**静默失败**的——
  代码里看不出异常，用户也收不到任何东西。探针页有「通知」区块显示当前授权状态、
  可一键跳系统设置，还有一个"弹一条测试通知"按钮用来立刻验证这条通道。
- **签名到期后表现**：7 天到期不重签的话 App 直接打不开（不是"功能受限"）。
