...
| `tools/win-spike/` | Windows 采集方 | Windows 采集层探针（WGC 通道、消息区定位、遮挡/最小化） |
| `desktop/` | Windows 采集方 | Windows 端副驾：截图读屏 → 判断 → 悬浮窗 → 复制（只读、不发送） |
| `docs/` | 主控 | 验收标准、报告 |
| `_reports/` | 所有人 | 每个任务的交付报告写这里 |

Windows 端另有两条自己的红线（与上面 6 条并列）：

8. **截图不落盘**：WGC 采到的帧只以内存对象存在，全程不写磁盘、不进日志、不上传（探针与调试图除外，且已被 `.gitignore` 排除）。
9. **不擅自动用户窗口**：微信最小化时零帧，只提示用户还原，不做"强制还原/置顶"这类改动别人窗口状态的操作。
